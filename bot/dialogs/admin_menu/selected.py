...
async def on_select_settings_categories(call: CallbackQuery, widget: Button, manager: DialogManager):
    await manager.start(states.EditCategory.select_category)


async def on_select_settings_items(call: CallbackQuery, widget: Button, manager: DialogManager):
...
async def on_select_category_for_edit(call: CallbackQuery, widget: Select, manager: DialogManager, item_id: str):
    try:
...
async def on_select_edit_category_param(call: CallbackQuery, widget: Select, manager: DialogManager, item_id: str):
    edit_params_mapping = {
...
async def on_change_category_name(message: Message, widget: ManagedTextInput, manager: DialogManager, message_text: str):
...
async def on_send_new_category(message: Message, widget: ManagedTextInput, manager: DialogManager, message_text: str):
...
async def on_confirm_delete_category(call: CallbackQuery, widget: Button, manager: DialogManager):
...
async def on_select_catgory_for_edit_item(call: CallbackQuery, widget: Select, manager: DialogManager, item_id: str):
    try:
        item_id = int(item_id)  # type: ignore
    except ValueError:
        logging.error(f"item_id is not integer: {item_id}")
...
async def on_start_add_item(call: CallbackQuery, widget: Button, manager: DialogManager):
    await manager.start(states.CreateItem.enter_name, data=manager.dialog_data)


async def on_select_item_for_edit(call: CallbackQuery, widget: Select, manager: DialogManager, item_id: str):
    try:
        item_id = int(item_id)  # type: ignore
    except ValueError:
        logging.error(f"item_id is not integer: {item_id}")
...
async def on_enter_new_item_name(message: Message, widget: ManagedTextInput, manager: DialogManager, message_text: str):
    new_name = message_text[:100]
    repo: Repo = manager.middleware_data['repo']
...
async def on_enter_new_item_price(message: Message, widget: ManagedTextInput, manager: DialogManager, message_text: str):
...
async def on_enter_new_item_description(message: Message, widget: ManagedTextInput, manager: DialogManager, message_text: str):
...
async def on_send_photo_for_new_item(message: Message, widget: MessageInput, manager: DialogManager):
    if message and message.photo:
        manager.dialog_data.update(photo_id=message.photo[-1].file_id, photo_unique_id=message.photo[-1].file_unique_id)
    else:
...
async def on_save_new_item(call: CallbackQuery, widget: Button, manager: DialogManager):
    ...
    try:
        ...
        await call.answer(f"Ошибка при добавлении товара. Попытайтесь самостоятельно исправить ошибку: {ex}", show_alert=True)
        return
