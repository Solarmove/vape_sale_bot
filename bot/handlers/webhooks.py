        ...
        async with session_factory() as session:
            ...
            if transaction:
                if payment_status =='finished':
                    await bot.send_message(transaction.user_id, f'Ваш заказ #{transaction.id} успешно оплачен!\n\nМы свяжемся с вами в ближайшее время для уточнения деталей.')
                    text_for_admins = ("Поступил новый заказ!\n\n"
                    ...
                    await bg_factory.bg(bot=bot, chat_id=transaction.user_id, user_id=transaction.user_id).done()
                    await bot.send_message(transaction.user_id, f'Ваш заказ #{transaction.id} находится в процессе оплаты. Пожалуйста, дождитесь подтверждения.')
